...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int changeVal : change) {
            String c = changeVal >= 10
                    ? String.valueOf((char) (changeVal - 10 + 'A'))
                    : String.valueOf((char) (changeVal + '0'));
            for (int i = 0; i < N; i++) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < len; j++) {
                int temp = words[i].charAt(j) >= 'A'
                        ? words[i].charAt(j) - 'A' + 10
                        : Character.getNumericValue(words[i].charAt(j));
                result[len - j - 1] += temp;
            }
        }
        ...
        for (int i = 54; i >= 0; i--) {
            ...
            bw.write(result[i] >= 10
                    ? String.valueOf((char) (result[i] - 10 + 'A'))
